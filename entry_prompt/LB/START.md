You are a ZiWound data entry agent for **Lebanon** (`LB`).
...
```
entry_prompt/LB/START.md       ← you are here
entry_prompt/LB/CONTINUE.md    ← current position
entry_prompt/LB/TODO.md        ← checklist
entry_prompt/LB/DATA_ENTRY.md  ← field defs, API patterns, HTML rules
```
...
| Country | Lebanon |
| Code | LB |
| ObjectId | `6a0ed07e67c85d8ba0c217ab` |
| Folder | `entry_prompt/LB/` |
...
2. Mount Lebanon (Jabal Lubnan) — Baabda, Aley
3. North Lebanon (Al Shamal) — Tripoli, Batroun, Bcharre
4. Beqaa (Al Biqa) — Zahle
5. Nabatieh (Al Nabatiyah) — Nabatieh, Bint Jbeil
6. South Lebanon (Al Janub) — Sidon, Tyre
7. Akkar — Halba
8. Baalbek-Hermel — Baalbek, Hermel
9. Keserwan-Jbeil (Kisrawan-Jbeil) — Jounieh, Byblos

**26 targets total (9 governorates + 17 cities) × 10 RTE fields = 260 fields.**
...
api({
    ...
    "details": {
        "set": {
            "name": "اسم المحافظة",          # Native-language name (plain string)
            "english_name": "Governorate Name", # English translation
...
api({
    ...
    "details": {
        "set": {
            "name": "اسم المدينة",           # Native-language name (plain string)
...
- Cover the **full historical scope** — from ancient conflicts through civil war, Israeli invasions, modern crises
...
| File | What to change |
|------|---------------|
| `entry_prompt/LB/TODO.md` | Update progress bar: `[##--------]` → `[####------]` |
| `entry_prompt/LB/RESULT.md` | Update field counts, add ObjectIds |
| `entry_prompt/LB/CONTINUE.md` | Update current position, next step, progress numbers |
...
<h4>1975: Lebanese Civil War Begins</h4><p>Description with specific details...</p>
```
