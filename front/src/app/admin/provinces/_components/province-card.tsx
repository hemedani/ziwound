...
export function ProvinceCard({ province, onDelete, index = 0 }: ProvinceCardProps) {
    ...
    >
      ...
      <div className="relative h-36 w-full bg-white/[0.02] overflow-hidden">
        ...
        <div className="absolute bottom-3 start-3 end-3">
          <h3 className="font-bold text-offwhite text-lg leading-tight truncate">{province.name}</h3>
          <p className="text-xs text-slate-body/80 truncate">{province.english_name}</p>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-4 text-xs text-slate-body mb-3">
          {province.cities && Array.isArray(province.cities) && (
            <span>{province.cities.length} {t("cities") || "cities"}</span>
          )}
        </div>
        ...
        <div className="flex items-center gap-1 pt-2 border-t border-white/[0.06]">
          ...
          <Button variant="ghost" size="sm" asChild className="text-slate-body hover:text-offwhite hover:bg-white/5 h-8 px-2">
            <Link href={`/admin/provinces/${province._id}/update-relations`}>
