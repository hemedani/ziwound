...
import { LayoutGrid, Table2, Plus, ArrowLeft, ArrowRight, Tags } from "lucide-react";
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-xl glass-strong p-4 border border-white/[0.06] flex items-center gap-3">
          <div className="h-10 w-10 rounded-lg bg-crimson/10 flex items-center justify-center shrink-0">
            <Tags className="h-5 w-5 text-crimson" />
          </div>
          <div>
            <p className="text-2xl font-bold text-offwhite">{totalCount}</p>
            <p className="text-xs text-slate-body">{t("totalTags") || "Total Tags"}</p>
          </div>
        </div>
        <div className="rounded-xl glass-strong p-4 border border-white/[0.06] flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold text-offwhite">{tags.filter(t => t.color).length}</p>
            <p className="text-xs text-slate-body">{t("tagsWithColor") || "Tags with color"}</p>
          </div>
        </div>
        <div className="rounded-xl glass-strong p-4 border border-white/[0.06] flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold text-offwhite">{tags.filter(t => t.icon).length}</p>
            <p className="text-xs text-slate-body">{t("tagsWithIcon") || "Tags with icon"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-body">
          {tags.length > 0 ? (
            <>
              <span className="text-offwhite font-medium">{tags.length}</span>
      ...
      {tags.length === 0 ? (
        <div className="rounded-2xl glass-light border border-white/[0.06] p-12">
          <EmptyState
            title={t("noTags") || "No tags found"}
            description={t("noTagsDescription") || "Try adjusting your filters or create a new tag."}
            action={
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {tags.map((tag, i) => (
            <TagCard key={tag._id} tag={tag} onDelete={handleDelete} index={i} />
          ))}
        </div>
      ) : (
