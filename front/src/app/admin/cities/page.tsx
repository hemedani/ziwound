...
import { AdminCitiesClient } from "./_components/admin-cities-client";
...
export default async function AdminCitiesPage({
  ...
  let cities: (citySchema & { province?: { _id?: string; name?: string; english_name?: string }; country?: { _id?: string; name?: string; english_name?: string } })[] = [];
  let error: string | null = null;
  if (response?.success) {
    cities = response.body || [];
  } else {
    error = response?.body?.message || "Failed to fetch cities";
  }
  ...
  const prevPageUrl = page > 1 ? `/admin/cities?page=${page - 1}${queryString}` : "";
  const nextPageUrl = cities.length >= 20 ? `/admin/cities?page=${page + 1}${queryString}` : "";
