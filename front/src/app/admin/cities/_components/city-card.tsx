...
import { MoreHorizontal, Pencil, Trash2, ImageUp, MapPin } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function CityCard({ city, onDelete, index = 0 }: CityCardProps) {
    ...
    >
      ...
      <div className="relative h-36 w-full bg-white/[0.02] overflow-hidden">
        {city.photo ? (
          ...
          <div className="h-full w-full flex items-center justify-center">
            <MapPin className="h-10 w-10 text-slate-body/20" />
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0a] via-transparent to-transparent" />
        <div className="absolute bottom-3 start-3 end-3">
          <h3 className="font-bold text-offwhite text-lg leading-tight truncate">{city.name}</h3>
          <p className="text-xs text-slate-body/80 truncate">{city.english_name}</p>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-4 text-xs text-slate-body mb-3">
          {city.province?.name && (
            <span className="truncate">{city.province.name}</span>
          )}
          {city.country?.name && (
            <span className="truncate text-gold/70">{city.country.name}</span>
          )}
        </div>
