...
export type CityFormSubmitData = CityFormValues & { countryId?: string; provinceId?: string; photoId?: string };

interface CityFormProps {
  initialData?: Partial<citySchema>;
...
export function CityForm({ initialData, onSubmit, onCancel, isEditing = false, countries = [], provinces = [] }: CityFormProps) {
  ...
  const [selectedProvinceId, setSelectedProvinceId] = useState<string | string[] | null>(
    (initialData as Record<string, { _id?: string }> | undefined)?.province?._id || null
  );
  ...
  const handleSubmit = (values: CityFormValues) => {
    startTransition(async () => {
      ...
      const provinceId = typeof selectedProvinceId === "string" ? selectedProvinceId : undefined;
      await onSubmit({ ...values, countryId, provinceId, photoId: photoId || undefined });
    });
  };
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
          ...
          <div className="rounded-2xl glass-strong p-5 border border-white/[0.06] space-y-6">
            ...
            <p className="text-xs text-slate-body/60">
              {t("cityInfoDescription") || "Document city war-related information in multiple languages"}
            </p>
