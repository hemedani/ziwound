...
import { LayoutGrid, Table2, Plus, ArrowLeft, ArrowRight, MapPin } from "lucide-react";
    ...
    >
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-body mt-1 text-sm">
              {t("citiesManagementDescription") || "Manage cities and their war documentation"}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-xl glass-strong p-4 border border-white/[0.06] flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-body">{t("totalCities") || "Total Cities"}</p>
          </div>
        </div>
        <div className="rounded-xl glass-strong p-4 border border-white/[0.06] flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold text-offwhite">{cities.filter(c => c.photo).length}</p>
            <p className="text-xs text-slate-body">{t("citiesWithPhoto") || "With photo"}</p>
          </div>
        </div>
        <div className="rounded-xl glass-strong p-4 border border-white/[0.06] flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold text-offwhite">
              {cities.filter(c => c.country).length}
            </p>
            <p className="text-xs text-slate-body">{t("countries") || "Countries"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-body">
          {cities.length > 0 ? (
            <>
              <span className="text-offwhite font-medium">{cities.length}</span>
      ...
      {cities.length === 0 ? (
        <div className="rounded-2xl glass-light border border-white/[0.06] p-12">
          <EmptyState
            title={t("noCities") || "No cities found"}
            description={t("noCitiesDescription") || "Try adjusting your filters or create a new city."}
            action={
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {cities.map((city, i) => (
            <CityCard key={city._id} city={city} onDelete={handleDelete} index={i} />
          ))}
        </div>
      ) : (
        <div className="rounded-2xl glass-light border border-white/[0.06] overflow-hidden">
          <CitiesTable cities={cities} onDelete={handleDelete} />
        </div>
      )}
