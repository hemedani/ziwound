...
export function CityEditClient({ city }: { city: citySchema }) {
    ...
    try {
      ...
      if (res?.success) {
        toast({
          ...
          description: t("cityUpdated") || "City has been updated successfully.",
        });
        ...
        toast({
          ...
          title: t("error") || "Error",
          description:
            res?.body?.message || res?.error || t("failedToUpdateCity") || "Failed to update city.",
        });
      }
    } catch {
      toast({
      ...
      <div className="relative overflow-hidden rounded-2xl glass-light border border-white/[0.06] p-6 md:p-8">
        ...
        <div className="relative">
          ...
          <p className="text-slate-body mt-1 text-sm">
            {t("editCityDescription") || "Update city information and war documentation"}
          </p>
        </div>
      </div>
