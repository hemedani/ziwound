      ...
      <div className="p-5 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            {category.icon ? (
              <span className="text-2xl leading-none shrink-0">{category.icon}</span>
            ) : (
              <div
            ...
            <div className="min-w-0">
              <h3 className="font-semibold text-offwhite truncate">{category.name}</h3>
              <div className="flex items-center gap-1.5 mt-0.5">
                <div
                  className="h-2.5 w-2.5 rounded-full"
                  style={{ backgroundColor: bgColor }}
                />
                <span className="text-xs text-slate-body/60 font-mono">{category.color || "—"}</span>
              </div>
            </div>
          </div>
        ...
        {category.description ? (
          <p className="text-sm text-slate-body line-clamp-2 leading-relaxed">{category.description}</p>
        ) : (
