...
interface AdminCountriesClientProps {
...
export function AdminCountriesClient({
  countries,
  totalCount,
  error,
  search,
    ...
    >
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-body mt-1 text-sm">
              {t("countriesManagementDescription") || "Manage countries and their war documentation"}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-xl glass-strong p-4 border border-white/[0.06] flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-body">{t("totalCountries") || "Total Countries"}</p>
          </div>
        </div>
        <div className="rounded-xl glass-strong p-4 border border-white/[0.06] flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold text-offwhite">{countries.filter(c => c.photo).length}</p>
            <p className="text-xs text-slate-body">{t("countriesWithPhoto") || "With photo"}</p>
          </div>
        </div>
        <div className="rounded-xl glass-strong p-4 border border-white/[0.06] flex items-center gap-3">
          ...
          <div>
            <p className="text-2xl font-bold text-offwhite">
              {countries.reduce((sum, c) => sum + (Array.isArray(c.provinces) ? c.provinces.length : 0), 0)}
            </p>
            <p className="text-xs text-slate-body">{t("totalProvinces") || "Provinces"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-body">
          {countries.length > 0 ? (
            <>
              <span className="text-offwhite font-medium">{countries.length}</span>
      ...
      {countries.length === 0 ? (
        <div className="rounded-2xl glass-light border border-white/[0.06] p-12">
          <EmptyState
            title={t("noCountries") || "No countries found"}
            description={t("noCountriesDescription") || "Try adjusting your filters or create a new country."}
            action={
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {countries.map((country, i) => (
            <CountryCard key={country._id} country={country} onDelete={handleDelete} index={i} />
          ))}
        </div>
      ) : (
